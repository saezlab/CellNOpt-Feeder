...
\subsection{Loading the toy example}

Loading the necessary packages and the toy example containing 28 interactions and 28 species (of which 2 are stimulated, 2 are inhibited and 6 are measrued). Data was generated across each measuremnt at each perturbation experimental condition for 60 time-points.
...
# loading the model
data("ToyModelPB", package="CNORfeeder")
# loading the data
data("ToyModelPBCNOlist", package="CNORfeeder")
# plotting the model
