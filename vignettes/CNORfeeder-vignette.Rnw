...
<<results, eval=FALSE, results=hide>>=
# model
...
\subsection{A. Loading the toy example and performing the initial ODE analysis}
...
# loading the model
data("ToyModel_Gene.RData", package="CNORfeeder")
# loading the data
data("CNOlistToy_Gene.RData", package="CNORfeeder")
...
# Alternatively: data("database.RData", package="CNORfeeder")
@
...
After reading the example, we perform an initial training of the model in the ODE formalism after first setting the optimization parameters.
...
opt_pars=parEstimationLBode(cnolist, model, method="essm", ode_parameters=ode_parameters, 
                            paramsSSm=paramsSSm)
simData = plotLBodeFitness(cnolist = cnolist, model = model, ode_parameters = opt_pars, 
                           transfer_function = 4)
@
